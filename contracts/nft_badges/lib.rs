...
/// multiple address to be execute from other contract 
#[ink::trait_definition]
pub trait AllowedContracts {
    #[ink(message)]
    fn ensure_operator(&self) -> Result<(), &'static str>;

    #[ink(message)]
    fn add_allowed_contract(&mut self, contract_address: AccountId) -> Result<(), &'static str>;

    #[ink(message)]
    fn remove_allowed_contract(&mut self, contract_address: AccountId) -> Result<(), &'static str>;

    #[ink(message)]
    fn is_contract_allowed(&self, contract_address: AccountId) -> bool;

    #[ink(message)]
    fn restricted_function(&self) -> Result<(), &'static str>;
}

#[ink::contract]
mod erc1155T {
    use super::*;
    use allowed_contracts::AllowedContracts;
    ...
    pub struct Contract {
        ...
        /// token ids tracking
        token_ids: Mapping<TokenId, bool>,
        /// 
        all_token_ids: Vec<TokenId>,
        total_supplies: Mapping<TokenId, Balance>,
         /// AllowedContracts reference
         allowed_contracts: allowed_contracts::AllowedContractsStruct,
    }
    ...
    impl Contract {
        ...
        #[ink(constructor)]
        pub fn new(allowed_contracts: allowed_contracts::AllowedContractsStruct) -> Self {
            Self {
                allowed_contracts,
                ..Default::default()
            }
        ...
        /// mint from Profile contract only
        #[ink(message)]
        pub fn mint_collectible_id(&mut self, token_id: TokenId, value: Balance) -> Result<(), InkEnvError> {
            self.allowed_contracts.ensure_operator()?;

            let caller = self.env().caller();
            self._mint(caller, token_id, value)?;
            Ok(())
        }

        /// mint directly by owner
        #[ink(message)]
        ...
        #[ink(message)]
        pub fn mint_batch(&mut self, token_ids: Vec<TokenId>, values: Vec<Balance>) -> Result<()> {
            // self.ensure_owner()?;
        ...
        fn _mint(&mut self, account: AccountId, token_id: TokenId, amount: Balance) -> Result<()> {
            
            // Ensure token ID tracking
            if !self.token_ids.get(token_id).unwrap_or(false) {
                self.token_ids.insert(token_id, &true);
            ...
            self.balances.insert((account, token_id), &new_balance);
            ...
            self.total_supplies.insert(token_id, &new_total_supply);
        ...
        #[ink(message)]
        pub fn balance_of_all(&self, account: AccountId) -> (Vec<TokenId>, Vec<Balance>, Vec<Balance>) {
            ...
            for token_id in &self.all_token_ids {
                ...
                balances.push(self.balances.get((account, *token_id)).unwrap_or(0));
                supplies.push(self.total_supplies.get(*token_id).unwrap_or(0));
            }

            (token_ids, balances, supplies)
        }
    ...
    impl super::AllowedContracts for Contract {
        /// Constructor that initializes the owner to the caller.
        #[ink(constructor)]
        pub fn new() -> Self {
            let caller = Self::env().caller();
            Self {
                owner: caller,
                allowed_contracts: Default::default(),
            }
        }

        /// Private function to check if the caller is the owner.
        fn ensure_owner(&self) -> Result<(), &'static str> {
        ...
        /// Private function to check if the caller is an allowed contract.
        fn ensure_operator(&self) -> Result<(), &'static str> {
            if !self.allowed_contracts.get(&self.env().caller()).unwrap_or(false) {
                return Err("Only allowed contracts can call this function");
            }
            Ok(())
        }
        ...
        #[ink(message)]
        pub fn add_allowed_contract(&mut self, contract_address: AccountId) -> Result<(), &'static str> {
            self.ensure_owner()?;
            self.allowed_contracts.insert(&contract_address, &true);
            Ok(())
        }

        /// Function to remove an allowed contract.
        #[ink(message)]
        pub fn remove_allowed_contract(&mut self, contract_address: AccountId) -> Result<(), &'static str> {
            self.ensure_owner()?;
            self.allowed_contracts.insert(&contract_address, &false);
            Ok(())
        }

        /// Function to check if a contract is allowed.
        #[ink(message)]
        pub fn is_contract_allowed(&self, contract_address: AccountId) -> bool {
            self.allowed_contracts.get(&contract_address).unwrap_or(false)
        }

        /// Example of a function restricted to allowed contracts.
        #[ink(message)]
        pub fn restricted_function(&self) -> Result<(), &'static str> {
...
// #[derive(scale::Encode, scale::Decode, Default, Clone, Copy, PartialEq, Eq, Debug)]
// #[cfg_attr(feature = "std", derive(scale_info::TypeInfo))]
// pub enum Error {
//     UnexistentToken,
// }